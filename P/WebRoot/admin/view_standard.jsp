		...
		<div class="templatemo-content-wrapper">
			<div class="templatemo-content">
				<ol class="breadcrumb">
					...
					<li class="active">结算批次与学分标准管理</li>
				</ol>
				...
				<div class="col-md-12 col-sm-12">
					...
					<!-- 常用功能：添加、导入、导出、批量删除 -->	
					<a href="javascript:;" class="btn btn-info" data-toggle="modal" data-target="#add_item">添加结算批次与学分标准</a> 
					<a href="javascript:;" class="btn btn-primary" data-toggle="modal" data-target="#import_item">导入结算批次与学分标准</a> 
					<a href="../standard/exportExcel" class="btn btn-success">导出结算批次与学分标准</a>
						...
						<tbody>
							...
							<c:if test="${!empty pu.show}">
								<c:forEach items="${pu.show}" var="item" varStatus="s">
									<tr
										onmouseover="updateItem('${item.id}','${item.credit}','${item.termContent}');">
										<td><input type="checkbox" name="subBox" value="${item.id}"/>${s.count}</td>	
										
										<td>${item.credit }</td>
										<td>${item.termContent }</td>
...
<!-- 以下为弹出区域，支持常用功能 -->
		...
		<div class="modal fade" id="import_item" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<form action="../standard/importExcel" method="POST" enctype="multipart/form-data">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal">
								<span aria-hidden="true">&times;</span><span class="sr-only">Close</span>
							</button>
							<h4 class="modal-title" id="myModalLabel">导入结算批次与学分标准数据</h4>
						</div>
						<div class="col-md-6 margin-bottom-15 cfg_add_input">
							<label for="exampleInputFile">选择结算批次与学分标准数据导入文件</label> 
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<form action="addStandard" method="post">
						...
						<div class="col-md-6 margin-bottom-15 cfg_add_input">
							<label for="firstName" class="control-label">需要学分数</label> <input
								type="text" class="form-control" id="item_credit_add"
								name="credit" placeholder="需要学分数">
						</div>
						<div class="col-md-6 margin-bottom-15 cfg_add_input">
							<label for="firstName" class="control-label">第几学期</label> <input
								type="text" class="form-control" id="item_termContent_add"
								name="termContent" placeholder="第几学期">
						</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<form action="updateStandard" method="post">
						...
						<div class="col-md-6 margin-bottom-15 cfg_add_input">
							<label for="firstName" class="control-label">需要学分数</label> <input
								type="text" class="form-control" id="item_credit_edit"
								name="credit" placeholder="需要学分数">
						</div>
						<div class="col-md-6 margin-bottom-15 cfg_add_input">
							<label for="firstName" class="control-label">第几学期</label> <input
								type="text" class="form-control" id="item_termContent_edit"
								name="termContent" placeholder="第几学期">
						</div>
		...
		<script type="text/javascript">
			function updateItem(item_id, item_credit, item_termContent) {
				$("#item_id_del").val(item_id);

				$("#item_id_edit").val(item_id);
				$("#item_credit_edit").val(item_credit);
				$("#item_termContent_edit").val(item_termContent);
			}
		</script>
				...
				var checklist = document.getElementsByName("subBox");
					...
					if(total == 0){
						...
						if(!confirm("您是否要删除 "+total+" 条记录？")){
							...
							if(ids != "") window.location.href="../standard/deleteAllStandard?ids="+ids;						
						}	
					}
				}
		</script>
